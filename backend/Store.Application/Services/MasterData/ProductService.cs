...
public class ProductService : IProductService
{
    ...
    private const string PosCacheKey = "products_pos";
    ...
    public async Task<IEnumerable<ProductListItemResponse>> GetAllForPosAsync(string? search, CancellationToken cancellationToken = default)
    {
        ...
        {
            return await _cache.GetOrCreateAsync(PosCacheKey, async entry =>
            {
                entry.SlidingExpiration = TimeSpan.FromMinutes(1);
                var products = await _productRepository.GetAllForPosAsync(null, cancellationToken);
                return products.Select(MapToListItemResponse).ToList();
            }) ?? Enumerable.Empty<ProductListItemResponse>();
        }

        var searchProducts = await _productRepository.GetAllForPosAsync(search, cancellationToken);
        return searchProducts.Select(MapToListItemResponse);
    }
